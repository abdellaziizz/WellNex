...
class UserRepository {
  ...
  // Private method to create initial health tracking
  Future<void> _createInitialHealthTracking(String userId, double weight, double height) async {
    try {
      final userDoc = _services.firestore.collection('users').doc(userId);
      
      // Create initial health tracking record
      await userDoc.collection('healthTracking').add({
      ...
      developer.log('Created initial health tracking for user: $userId');
    } catch (e) {
      developer.log('Error creating initial health tracking: $e');
      // Don't throw - we don't want to fail user creation if health tracking fails
    }
  }

  // Private method to create initial diet plan
  Future<void> _createInitialDietPlan(String userId, String userType, double weight) async {
    try {
      final userDoc = _services.firestore.collection('users').doc(userId);
      
      // Calculate daily calories based on user type and weight
      int dailyCalories = _calculateDailyCalories(userType, weight);
      
      // Create initial diet plan
      await userDoc.collection('dietPlans').add({
            ...
            'foods': [
              {
                ...
                'portion': '1 serving',
                'calories': (dailyCalories * 0.25).round() // 25% of daily calories
              }
            ]
          },
          {
            ...
            'foods': [
              {
                'name': 'Default lunch',
                'portion': '1 serving',
                'calories': (dailyCalories * 0.35).round() // 35% of daily calories
              }
            ]
          },
          {
            ...
            'foods': [
              {
                'name': 'Default dinner',
                'portion': '1 serving',
                'calories': (dailyCalories * 0.40).round() // 40% of daily calories
              }
            ]
          }
        ]
      ...
      developer.log('Created initial diet plan for user: $userId');
    } catch (e) {
      developer.log('Error creating initial diet plan: $e');
      // Don't throw - we don't want to fail user creation if diet plan fails
    }
  }

  // Helper method to calculate daily calories based on user type and weight
  int _calculateDailyCalories(String userType, double weight) {
    switch (userType.toLowerCase()) {
      case 'weightmanagement':
        return (weight * 25).round(); // Lower calories for weight management
      case 'fitness':
        return (weight * 35).round(); // Higher calories for fitness training
      case 'elder':
        return (weight * 23).round(); // Lower calories for elderly
      case 'busy':
        return (weight * 28).round(); // Moderate calories for busy lifestyle
      case 'beginner':
      default:
        return (weight * 30).round(); // Standard calories for beginners
    }
  }
  ...
  Future<User> createUserWithType({
    ...
    try {
      ...
      // Create initial subcollections for the user
      await Future.wait([
        _createInitialDietPlan(id, userType, weight),
        _createInitialHealthTracking(id, weight, height)
      ]);
